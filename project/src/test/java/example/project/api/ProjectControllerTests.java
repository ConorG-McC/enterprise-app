...
public class ProjectControllerTests {
    private String API_BASE_URL = "http://localhost:8901/restaurants/";
    private String MOCK_ADMIN_TOKEN = "Admin";
    private String VALID_RESTAURANT_ID = "1234";
    ...
    private Project createValidRestaurantWithTasks(){
        ...
        Project project =  Project.projectOf(VALID_RESTAURANT_ID,"The Balti");
    ...
    @Test
    @DisplayName("Pass a valid restaurant id to view a particular restaurant's details, display the response in JSON")
    void test01() throws Exception {
        ...
        restaurantResponse.setProjectId(VALID_RESTAURANT_ID);
        restaurantResponse.setName("Restaurant");
        //mock behaviour
        when(identityService.isAdmin(MOCK_ADMIN_TOKEN)).thenReturn(true);
        when(projectQueryHandler.getProject(VALID_RESTAURANT_ID)).thenReturn(Optional.of(restaurantResponse));

        //Check the format in generated_requests to ensure the json path keys are correct
        mockMvc.perform(get(API_BASE_URL.concat(VALID_RESTAURANT_ID))
                        .header("Authorization", MOCK_ADMIN_TOKEN))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("restaurantId").value(restaurantResponse.getProjectId()))
    ...
    void test02() throws Exception {
        ...
        mockMvc.perform(get(API_BASE_URL.concat("all"))
                ...
                .andExpect(jsonPath("$[0].menuItems[0].id", equalTo((int) restaurant.getTasks().get(0).getId())))
                .andExpect(jsonPath("$[0].menuItems[0].name", equalTo(restaurant.getTasks().get(0).getName())))
                .andExpect(jsonPath("$[0].menuItems[0].price", equalTo(restaurant.getTasks().get(0).getHours()))
                );
    }
    ...
    void test03() throws Exception {
        ...
        restaurantMenuResponse.setProjectId(VALID_RESTAURANT_ID);
        restaurantMenuResponse.setName("Restaurant");
        restaurantMenuResponse.setTasks(List.of(createValidMenuItem()));
        //mock behaviour
        when(identityService.isAdmin(MOCK_ADMIN_TOKEN)).thenReturn(true);
        when(projectQueryHandler.getProjectTasks(VALID_RESTAURANT_ID)).thenReturn(Optional.of(restaurantMenuResponse));

        //Check the format in generated_requests to ensure the json path keys are correct
        mockMvc.perform(get(API_BASE_URL.concat("menuItems/" + VALID_RESTAURANT_ID))
                        .header("Authorization", MOCK_ADMIN_TOKEN))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("restaurantId").value(restaurantMenuResponse.getProjectId()))
                .andExpect(jsonPath("name", equalTo(restaurantMenuResponse.getName())))
                //expected 1L was 1 (so need to cast from long to int
                .andExpect(jsonPath("menuItems[0].id", equalTo((int) restaurantMenuResponse.getTasks().get(0).getId())))
                .andExpect(jsonPath("menuItems[0].name", equalTo(restaurantMenuResponse.getTasks().get(0).getName())))
                .andExpect(jsonPath("menuItems[0].price", equalTo(restaurantMenuResponse.getTasks().get(0).getHours()))
                );
    }
